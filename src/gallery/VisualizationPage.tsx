...
import { ExplorationVisualization } from '../visualizations/ExplorationVisualization'
...
export function VisualizationPage({ slug, onBack }: VisualizationPageProps) {
  ...
  return <main className="gallery-item-page">
    <section className="embed-wrapper" aria-label={`${entry.title} visualization`}>
      {isDemographics ? <DemographicProfile /> : <ExplorationVisualization entry={entry} />}
    ...
    <section id="visualization-details" className="details-section">
      <div className="details-inner">
        <div className="details-header"><div className="details-thumb"><img src={entry.thumbnailSrc} alt="" /></div><h1 className="details-title">{entry.title}</h1></div>
        {isDemographics ? <>
        ...
        </> : <><p>{entry.summary}</p><h2>Interaction model</h2><p>This route includes a responsive SVG exploration surface with point/segment selection, hover feedback, panning, zoom controls, reset, and a time slider. Its geometry, controls, and data treatment are isolated so this module can be refined against the source visual independently.</p></>}
